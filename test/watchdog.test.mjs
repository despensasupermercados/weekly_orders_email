...
const base = [
  ...
  ["note LIKE 'Azamara MLS%' AND note NOT LIKE 'Azamara MLS REFUSED%'", { ts: '2026-09-01 08:00:00' }],
...
const noRoute = await runWatchdog(
  { HON: fakeDb(withRow("sender NOT IN ('cron', 'watchdog')", { n: 0, ts: null })
      .map(([f, v]) => (f === "note LIKE 'Azamara MLS%' AND note NOT LIKE 'Azamara MLS REFUSED%'" ? [f, { ts: null }] : [f, v]))) },
  TODAY, { repair: false });
...
const noMls = await runWatchdog(
  { HON: fakeDb(withRow("note LIKE 'Azamara MLS%' AND note NOT LIKE 'Azamara MLS REFUSED%'", { ts: null })) }, TODAY, { repair: false });
...
const staleMls = await runWatchdog(
  { HON: fakeDb(withRow("note LIKE 'Azamara MLS%' AND note NOT LIKE 'Azamara MLS REFUSED%'", { ts: '2026-06-01 08:00:00' })) },
  TODAY, { repair: false });
...
const freshMls = await runWatchdog(
  { HON: fakeDb(withRow("note LIKE 'Azamara MLS%' AND note NOT LIKE 'Azamara MLS REFUSED%'",
